...
from ml_train_all import (load_data, feature_engineering,
                           get_feature_cols, build_model, STATS,
                           METRICS_PATH, BEST_PARAMS_PATH)
...
def evaluate_models():
    ...
    # Stampa tabella
    header = f"{'Stat':12s} {'RF':>6s} {'XGB':>6s} {'HGB':>6s} {'Vincit.':>7s}  {'Champion':>14s}  {'Optuna':>14s}  {'Aggiornato':>12s}"
    print(header)
    print("-" * len(header))
    for r in rows:
        print(f"{r['Stat']:12s} {r['RF']:>6.3f} {r['XGB']:>6.3f} {r['HGB']:>6.3f} "
