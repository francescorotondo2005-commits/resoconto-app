...
// Configure delay (e.g. 1500ms is safe and relatively fast)
const DELAY_MS = 1500;
...
async function run() {
  ...
  for (const dateRow of dates) {
    ...
    // Fetch SofaScore events for this date
    ...
    for (const dbMatch of dbMatches) {
      ...
      if (!statsData || !statsData.statistics) {
        console.log(`  [WARN] No stats found for Sofa ID: ${sofaEvent.id}. Updating basic goals only.`);
        // Set basic default dummy structure to prevent repeated processing
        await db.execute({
    ...
    // Check if we hit limit or if we want to limit this run
    if (totalUpdated >= 100) {
      console.log(`\nReached limit of 100 updates for this safety test run.`);
      break;
    }
  }
  
  console.log(`\n=================================================`);
  console.log(` SofaScore Backfill Session Finished! Updated ${totalUpdated} matches.`);
  console.log(`=================================================`);
}
