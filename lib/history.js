...
export function calcHistorySummary(homeTeam, awayTeam, referee, betName, matchesBefore) {
  ...
  // --- Squadra Casa (solo partite in casa) ---
  let homeTotal = 0, homeHits = 0;
  for (const m of matchesBefore) {
    if (m.home_team?.toLowerCase() === homeTeam?.toLowerCase()) {
      homeTotal++;
      if (checkOutcome(parsed, m, 'home', trackHome)) homeHits++;
    }
  }

  // --- Squadra Ospite (solo partite in trasferta) ---
  let awayTotal = 0, awayHits = 0;
  for (const m of matchesBefore) {
    if (m.away_team?.toLowerCase() === awayTeam?.toLowerCase()) {
      awayTotal++;
      if (checkOutcome(parsed, m, 'away', trackAway)) awayHits++;
    }
  }

  // --- Arbitro (solo se rilevante per la statistica) ---
  ...
  const homePct = homeTotal >= MIN_SAMPLE_TEAM ? homeHits / homeTotal : null;
  const awayPct = awayTotal >= MIN_SAMPLE_TEAM ? awayHits / awayTotal : null;
  ...
  return {
    homePct: homePct !== null ? Math.round(homePct * 1000) / 1000 : null,
    homeSample: homeTotal,
    awayPct: awayPct !== null ? Math.round(awayPct * 1000) / 1000 : null,
    awaySample: awayTotal,
