...
async function initSchema(db) {
  await db.executeMultiple(`
    ...
    CREATE TABLE IF NOT EXISTS prediction_log (
      ...
      mae_away REAL,
      created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
      UNIQUE(match_key, stat_name)
    );

    CREATE TABLE IF NOT EXISTS match_ai_context (
      match_key TEXT PRIMARY KEY,
      home_team TEXT NOT NULL,
      away_team TEXT NOT NULL,
      date TEXT NOT NULL,
      ...
      key_players_missing_home TEXT,
      key_players_missing_away TEXT,
      raw_analysis TEXT,
      created_at DATETIME DEFAULT CURRENT_TIMESTAMP
    );
    ...
    CREATE INDEX IF NOT EXISTS idx_prediction_log_match ON prediction_log(match_key);
    CREATE INDEX IF NOT EXISTS idx_match_ai_context_date ON match_ai_context(date);
  `);

  // Migration: aggiunta colonna in_gioco a pending_matches (idempotente)
  try {
    await db.execute('ALTER TABLE pending_matches ADD COLUMN in_gioco INTEGER DEFAULT 0');
  } catch {
    // Colonna già esistente — ignora
  }

  // Migration: aggiunta colonna ev_ml e cv a bets e backtest_bets
  try { await db.execute('ALTER TABLE bets ADD COLUMN ev_ml REAL'); } catch {}
  try { await db.execute('ALTER TABLE backtest_bets ADD COLUMN ev_ml REAL'); } catch {}
  try { await db.execute('ALTER TABLE backtest_bets ADD COLUMN cv REAL'); } catch {}

  // Migration: storico e forma per backtest_bets (idempotente)
  const histCols = [
    'ALTER TABLE backtest_bets ADD COLUMN hist_score REAL',
    'ALTER TABLE backtest_bets ADD COLUMN home_hist_pct REAL',
    'ALTER TABLE backtest_bets ADD COLUMN home_hist_sample INTEGER',
    'ALTER TABLE backtest_bets ADD COLUMN away_hist_pct REAL',
    'ALTER TABLE backtest_bets ADD COLUMN away_hist_sample INTEGER',
    ...
    'ALTER TABLE backtest_bets ADD COLUMN ref_hist_pct REAL',
    'ALTER TABLE backtest_bets ADD COLUMN ref_hist_sample INTEGER',
    'ALTER TABLE backtest_bets ADD COLUMN form_home_pct REAL',
    'ALTER TABLE backtest_bets ADD COLUMN form_home_n INTEGER',
    'ALTER TABLE backtest_bets ADD COLUMN form_away_pct REAL',
    'ALTER TABLE backtest_bets ADD COLUMN form_away_n INTEGER',
    ...
    'ALTER TABLE backtest_bets ADD COLUMN form_ref_pct REAL',
    'ALTER TABLE backtest_bets ADD COLUMN form_ref_n INTEGER',
  ];
  for (const sql of histCols) {
    try { await db.execute(sql); } catch { /* colonna già esistente */ }
  }
