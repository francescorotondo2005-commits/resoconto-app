...
function parseLabel(label) {
  ...
  else if (lowered.startsWith('tip')) stat = 'tip';
...
export function getMatchStatValue(match, typeStat, scope) {
  ...
  if (isCartellini) {
    ...
    homeVal = Math.max(0, Number(match.away_sot) - Number(match.home_goals));
    awayVal = Math.max(0, Number(match.home_sot) - Number(match.away_goals));
  } else {
    const statMap = {
