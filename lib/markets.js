...
export function getAllMarkets() {
  ...
  markets.push(...generateOverUnder('tiri', 'TIRI TOTALI', 19, 29, 1, 'totale'));
  markets.push(...generateOverUnder('tiri', 'TIRI CASA', 6, 19, 1, 'casa'));
  markets.push(...generateOverUnder('tiri', 'TIRI OSPITE', 6, 18, 1, 'ospite'));

  // === TIRI IN PORTA === (1X2 -> Totali -> Casa -> Ospite)
  markets.push(...generate1X2('tip', 'TIRI IN PORTA'));
  markets.push(...generateOverUnder('tip', 'TIRI IN PORTA TOTALI', 5, 10, 1, 'totale'));
  markets.push(...generateOverUnder('tip', 'TIRI IN PORTA CASA', 0, 8, 1, 'casa'));
  markets.push(...generateOverUnder('tip', 'TIRI IN PORTA OSPITE', 0, 8, 1, 'ospite'));

  // === FALLI === (1X2 -> Totali -> Casa -> Ospite)
  markets.push(...generate1X2('falli', 'FALLI'));
  markets.push(...generateOverUnder('falli', 'FALLI TOTALI', 18, 28, 1, 'totale'));
  ...
  markets.push(...generateOverUnder('corner', 'CORNER CASA', 0, 9, 1, 'casa'));
  markets.push(...generateOverUnder('corner', 'CORNER OSPITE', 0, 9, 1, 'ospite'));

  // === CARTELLINI === (1X2 -> Totali -> Casa -> Ospite)
  markets.push(...generate1X2('cartellini', 'CARTELLINI'));
  markets.push(...generateOverUnder('cartellini', 'CARTELLINI TOTALI', 1, 7, 1, 'totale'));
