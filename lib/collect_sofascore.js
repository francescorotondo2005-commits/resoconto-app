...
async function processDate(targetDate) {
    ...
    const dbMatchesRes = await db.execute({
        sql: `SELECT id, home_team, away_team FROM matches WHERE date(date) = ? AND (home_xg IS NULL OR home_passes IS NULL)`,
        args: [targetDate]
    });
...
if (args.includes('--backfill')) {
    ...
    (async () => {
        const db = getDbClient();
        const res = await db.execute(`SELECT DISTINCT date(date) as d FROM matches WHERE home_passes IS NULL ORDER BY date(date) DESC`);
