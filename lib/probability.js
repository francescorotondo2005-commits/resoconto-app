...
/**
 * Calibrates raw probability using Platt scaling (sigmoid mapping)
 * to reduce model overconfidence and align with real-world frequencies.
 */
function calibrateProbability(prob, marketType) {
  if (prob <= 0.001) return 0;
  if (prob >= 0.999) return 1;
  
  let a = 0.92; // Default scaling factor
  let b = 0.0;
  
  if (marketType === '1x2') {
    a = 0.88; // Slightly stronger calibration for 1X2 outcomes
...
export function PROB_BINOM_NEG_ML(linea, ev, varianza, mercato) {
  try {
    ...
    if (v <= m || isNaN(v)) {
      ...
      rawProb = mercato.toLowerCase() === 'over' ? (1 - probUnder) : probUnder;
    } else {
    ...
    return calibrateProbability(rawProb, 'over_under');
  } catch (e) {
  ...
  try {
    ...
    for (let i = 0; i <= 25; i++) {
      ...
      for (let j = 0; j <= 25; j++) {
        ...
        let probCombinata = pCasa * pOspite;

        // Dixon-Coles Bivariate Poisson adjustment for low scores (Fase 5)
        if (i <= 1 && j <= 1) {
          const rho = -0.06; // Dixon-Coles dependence parameter
          const lambda = Number(evCasa);
          const mu = Number(evOspite);
          let tau = 1.0;
    ...
    let rawProb = 0;
    if (esito === '1') rawProb = prob1 / totale;
    else if (esito.toUpperCase() === 'X') rawProb = probX / totale;
    else if (esito === '2') rawProb = prob2 / totale;
    
    return calibrateProbability(rawProb, '1x2');
  } catch (e) {
